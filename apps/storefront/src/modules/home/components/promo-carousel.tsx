...
export default function PromoCarousel({ ads = [] }: { ads?: PromotionalAd[] }) {
  ...
  const livePromos = ads.filter((ad) => ad.placement === "homepage_carousel").map((ad) => ({ ...ad, eyebrow: ad.eyebrow ?? "Featured offer", copy: ad.description ?? "Shop the latest offers from Coast Tech.", cta_label: ad.cta_label ?? "Shop now" }))
  ...
  return (
    <section className="content-container py-5 small:py-8">
      <div className="relative min-h-[192px] overflow-hidden rounded-xl bg-[#161616] px-6 py-7 text-white shadow-sm small:min-h-[478px] small:px-12 small:py-20">
        ...
        <div className="relative max-w-xl"><p className="inline-block rounded-full bg-white/20 px-3 py-1 text-xs font-bold uppercase tracking-wider">{promo.eyebrow}</p><h1 className="mt-4 text-3xl font-extrabold leading-tight small:text-5xl">{promo.title}</h1>{promo.discount_label ? <p className="mt-2 text-lg font-bold text-yellow-200">{promo.discount_label}</p> : null}<p className="mt-2 text-sm text-gray-100 small:text-base">{promo.copy}</p><LocalizedClientLink href={promo.href} className="mt-5 inline-block rounded-full bg-white px-6 py-3 text-sm font-bold text-orange-700 hover:bg-orange-50">{promo.cta_label}</LocalizedClientLink></div>
